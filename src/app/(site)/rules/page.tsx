...
export default async function RulesPage() {
  ...
  return <article className="shell max-w-2xl py-10">
    ...
    <div className="mt-8 space-y-8 text-sm leading-7">
      <section id="entry-fees"><h2 className="text-lg font-semibold">01 / Entering a season</h2><p className="text-subtle mt-2">{season ? `${season.name}: ${season.capacity} products at ${formatMoney(season.entryPriceCents, season.currency)} per entry.` : "Capacity and pricing are published when a season opens."} Checkout is simulated, so no money is charged. Paying takes your spot straight away — there is no waiting on approval. A season starts only once every spot is filled: a short field waits rather than playing a smaller bracket. The first round then opens automatically and runs for {season?.defaultRoundHours ?? 24} hours.</p></section>
