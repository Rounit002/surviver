...
/** A complete 35-product field, ready to play. */
function counts(overrides: Partial<StartCounts> = {}): StartCounts {
  return { status: "REGISTRATION_OPEN", capacity: 35, claimed: 35, ready: 35, ...overrides };
}
...
test("holds a season that is one or two spots short", () => {
  for (const claimed of [33, 34]) {
    ...
    assert.equal(readiness.remaining, 35 - claimed);
    assert.match(readiness.blockedReason ?? "", new RegExp(`^${35 - claimed} of 35 spots are still open`));
  }
});
...
test("refuses a full field holding a slot that never became startable", () => {
  ...
  // it still holds capacity, so starting here would field 34 in a 35 bracket.
  const readiness = evaluateStartReadiness(counts({ ready: 34 }));
  assert.equal(readiness.canStart, false);
  assert.equal(readiness.isFull, true);
  assert.equal(readiness.blockedReason, "Only 34 of 35 entries are approved and paid.");
});
