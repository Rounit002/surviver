...
| F05 | Closed | Database-backed atomic rate limiting on signup, sign-in, entry creation, site lookup, impressions, outbound visits, rally landings, verification email and the dev payment action. Streaming byte caps on the impression and webhook bodies. Route parameters are shape-bounded before reaching the database. | `src/lib/security/request.ts`, `src/app/api/impressions/route.ts:8`, `src/app/api/webhooks/dodo/route.ts:11`, `src/app/rally/[code]/route.ts:19`. |
...
| F14 | Substantially addressed | Entry now requires a verified email account and the submitted address must match it, so a third party can no longer create entries under someone else's address. Campaign tokens are stored hashed with expiry and revocation columns. Unpaid entries stop blocking a URL after two hours and are swept to `WITHDRAWN`. The capability no longer travels in the provider return URL: it is held in an HttpOnly cookie and re-formed on our own origin at `/enter/complete`. | `src/lib/products/actions.ts:127`, `:169`, `:196-199`, `src/app/(site)/enter/complete/route.ts`, `src/lib/security/retention.ts`. **Residual:** no self-service capability rotation for founders. |
| F15 | Closed | Sessions now carry both an absolute limit (7 days founder, 12 hours admin) and an idle limit (24 hours / 2 hours) that slides forward but never past the absolute one. Verified email is required to sign in; administrators must additionally pass TOTP. Email verification revokes all existing sessions. | `src/lib/auth/session.ts:10-30`, `:84-100`, `src/lib/auth/mfa.ts`, `src/app/(auth)/verify-email/[token]/route.ts:22`. |
