...
| F05 | **Reopened by product decision** | The byte caps and the shape-bounded route parameters remain. The rate limiting this row originally recorded was **removed** on 19 September 2026 (`Remove request limits and private IP filtering`, 4fe9493), along with the `rate_limit_buckets` table. Nothing now throttles sign-in, signup, entry creation or the site-metadata lookup. | `src/lib/security/request.ts`, `src/app/api/impressions/route.ts:8`, `src/app/api/webhooks/dodo/route.ts:11`, `src/app/rally/[code]/route.ts:19`. **Open:** online password guessing against `/login` is unbounded, and `lookupSiteAction` is an unauthenticated outbound fetcher anyone can drive. |
...
| S05 | Medium | Rate limiting was removed wholesale in 4fe9493. Online password guessing against `/login` is unbounded, and `lookupSiteAction` is an unauthenticated outbound fetcher any third party can drive. See F05 above. | **Open, deliberate.** Restoring it needs a storage decision, so it is left to the owner. |
| S06 | Low | Unknown season numbers answer `200` while rendering the not-found page. The `(site)` loading boundary streams the shell before `notFound()` runs, so the status is already committed. Public and indexable, unlike the noindex campaign and checkout routes, which have the same shape. | **Open.** Fixing it means moving the loading boundary off `/seasons/[number]`. |
| S07 | Low | `/entry/[token]` still accepts a plaintext `manageToken` and upgrades it to a hash on read. A pre-hash database dump remains replayable until every legacy row has been visited once. | **Open, by design** (migration path). |
| S08 | Operational | A local `next dev` was running against the remote production database. A test entry there writes real rows into the live season. | Reported to the owner; the local server was rebound to the database in `.env`. |
...
| `npx eslint src prisma prisma.config.ts` | Clean. |
| `npm test` | 33 passed, 0 failed, 0 skipped, with `TEST_DATABASE_URL` pointed at `surviver_test`. |
| `npm run build` | Succeeds. |
| `npx prisma migrate status` | `20260919210000_payment_charged_total` had never been applied locally; applied during this pass. |
| End-to-end entry | URL pasted on the hero, metadata read, checkout taken, payment settled, entry `UPCOMING`, product `APPROVED`, and the listing visible on the home board, `/board` and the founder dashboard. |
| Public routes | All render; `/does-not-exist` answers 404. See S06 for the soft-404 case. |
