...
| F05 | Closed | Database-backed atomic rate limiting on signup, sign-in, entry creation, site lookup, impressions, outbound visits, rally landings and the dev payment action. Streaming byte caps on the impression and webhook bodies. Route parameters are shape-bounded before reaching the database. | `src/lib/security/request.ts`, `src/app/api/impressions/route.ts:8`, `src/app/api/webhooks/dodo/route.ts:11`, `src/app/rally/[code]/route.ts:19`. |
